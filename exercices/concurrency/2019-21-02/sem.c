...
{
    sem_t mutex;
    sem_t operaio[N_OPERAI];
    sem_t attesa_riparazione;
    sem_t attesa_ufficio;

    int coda_ufficio, stato_ufficio;
} o;
...
void init_officina(struct officina_t *officina)
{
    ...
    for (int i = 0; i < N_OPERAI; i++)
    {
        sem_init(&officina->operaio[i], 0, 0);
    }
    officina->coda_ufficio = officina->stato_ufficio = 0;
}
...
{
    // sveglio un cliente in attesa di riparazione
